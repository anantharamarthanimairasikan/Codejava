...
public class AccountControllerTest {
    ...
    public void testMakePayment_Success() throws AccountNotFoundException {
        ...
        paymentReq.setCustomerid(1);
        paymentReq.setAmount(50);
        paymentReq.setPaymethod(Epaymethod.BANK_TRANSFER);
 
        when(accountService.findAccount(paymentReq.getCustomerid().longValue())).thenReturn(account);
        ResponseEntity<Payments> response = accountController.makepayment(paymentReq);
 
        verify(accountService).findAccount(paymentReq.getCustomerid().longValue());
        assertEquals(HttpStatus.OK, response.getStatusCode());
    ...
    public void testMakePayment_AccountNotFound() throws AccountNotFoundException {
        PaymentReq paymentReq = new PaymentReq();
        paymentReq.setCustomerid(2); // Account id that doesn't exist
        paymentReq.setAmount(50);
        paymentReq.setPaymethod(Epaymethod.BANK_TRANSFER);
 
        when(accountService.findAccount(paymentReq.getCustomerid().longValue())).thenThrow(new AccountNotFoundException("Account not found"));
        ResponseEntity<Payments> response = accountController.makepayment(paymentReq);
